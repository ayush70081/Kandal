...
const Leaderboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await api.get('/auth/leaderboard', { params: { limit: 100 } });
        if (res.data.success) {
  ...
  return (
    <div className="container" style={{ padding: '1.5rem' }}>
      <h2 style={{ margin: 0, marginBottom: '1rem' }}>Leaderboard</h2>
      ...
      <div className="admin-table-container">
        <table className="admin-table">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan="4" style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>No data</td>
              </tr>
            ) : (
              rows.map(u => (
                <tr key={u._id}>
